...
public class Application extends JavaController {
    ...
    public static Result rollDice() {
        ...
        observer.asMap().get(getSession()).setChangePlayer(false);
        ...
        if (!controllers.asMap().get(currentSession).isCorrectOption(UserAction.START_TURN)) {
            ...
            return ok(getMessage(observer.asMap().get(getSession()).getLastMessage()));
        }
    ...
    private static Result handlePrisonRoll() {
        ...
        if (!controllers.asMap().get(currentSession).isCorrectOption(UserAction.ROLL_DICE)) {
            ...
            return ok(getMessage(observer.asMap().get(currentSession).getLastMessage()));
        }
    ...
    public static String msg(String msg) {
        observer.asMap().get(getSession()).setLastMessage(msg);
        return msg;
    }

    public static Result getLastMessage() {
        return ok(observer.asMap().get(getSession()).getLastMessage());
    }
